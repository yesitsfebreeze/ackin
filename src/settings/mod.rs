...
//! The declaration format lives in [`transport::settings`]; the host's own keys
...
pub use transport::settings::{declared, defaults, get, merge, set, undeclared, Kind, Spec, Specs};
...
pub fn apply(specs: &Specs, config: Json, at: &str) -> Result<Json> {
	transport::settings::apply(specs, config, at).map_err(Error::Settings)
}
